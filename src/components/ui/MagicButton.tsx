...
export function MagicButton({ children, className = '', ...props }: MagicButtonProps) {
  return (
    ...
    >
      ...
      />
      <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-xl bg-(--color-accent) px-3 py-1 text-sm font-medium text-white backdrop-blur-3xl">
        {children}
      </span>
    </HeroButton>
  )
