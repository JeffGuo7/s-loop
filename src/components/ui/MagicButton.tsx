...
export function MagicButton({ children, className = '', ...props }: MagicButtonProps) {
  return (
    ...
    >
      ...
      />
      {/* Inner button — uses HeroUI primary for solid dark mode contrast */}
      <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-xl bg-primary text-primary-foreground px-3 py-1 text-sm font-medium backdrop-blur-3xl">
        {children}
      </span>
    </HeroButton>
  )
