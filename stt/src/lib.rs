...
//! setup, permissions, and how a final transcript is inserted.
...
impl Dictation {
    pub fn new(model_dir: impl Into<PathBuf>) -> Self {
        // CoreAudio streams are !Send, so one dedicated thread owns the CPAL stream on every OS.
    ...
    pub fn install_default_model_with_progress(
        ...
        let result = (|| {
            ...
            let response = agent
                ...
                .call()
                .map_err(|error| format!("Could not download the local voice model: {error}"))?;
            let mut input = response.into_reader();
            let mut output = fs::File::create(&partial)
                .map_err(|error| format!("Could not save the local voice model: {error}"))?;
            ...
            loop {
                ...
                let count = input.read(&mut buffer).map_err(|error| {
                    format!("Could not download the local voice model: {error}")
                })?;
                if count == 0 {
                    break;
                }
            ...
            output.flush().map_err(|error| {
                format!("Could not finish saving the local voice model: {error}")
            })?;
            drop(output);
            ...
            if self.model_path.exists() {
                fs::remove_file(&self.model_path)
                    .map_err(|error| format!("Could not replace the local voice model: {error}"))?;
            }
    ...
    pub fn mark_test_passed(&self) -> Result<(), String> {
        ...
        fs::write(&self.ready_marker_path, b"ready")
            .map_err(|error| format!("Could not save the voice input test result: {error}"))
    }
    ...
    pub fn input_level(&self) -> f32 {
        ...
        let mean_square =
            tail.iter().map(|sample| sample * sample).sum::<f32>() / tail.len() as f32;
...
fn verify_model_file(path: &Path) -> Result<(), String> {
    let metadata = fs::metadata(path)
        .map_err(|error| format!("Could not read the local voice model: {error}"))?;
...
fn write_verification_marker(model_path: &Path, marker_path: &Path) -> Result<(), String> {
    ...
    fs::write(marker_path, format!("{DEFAULT_MODEL_SHA256}\n{modified}\n"))
        .map_err(|error| format!("Could not record voice model verification: {error}"))
}
...
fn transcribe(model_path: &Path, samples: &[f32]) -> Result<String, String> {
    ...
    .map_err(|error| format!("Could not load the local voice model: {error}"))?;
...
mod tests {
    ...
    #[test]
    fn default_model_size_matches_multilingual_base() {
        assert_eq!(DEFAULT_MODEL_BYTES, 147_951_465);
    }

    #[test]
    fn readiness_requires_verification_and_microphone_test() {
